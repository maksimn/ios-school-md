...
__Фунциональные преобразования__ - `map`, `filter`, `reduce`.

__Операции со списками__ - f.e. "взять 1-й (2-й | 5-й) элемент".

__Работа с потоками и очередями__ - вынос тяжелых или долгих операций из главного потока в фон.

__Шедулинг и работа со временем__ - интеграция с Run Loop, Dispatch Queue, Timer, timeout etc.
...
В изложенном выше примере вместо `map` мб лучше `compactMap`.

---

__Примеры задач, хорошо решаемых с использованием реактивщины.__ Это задачи с асинхронностью.

__#1__: Пусть пользователю для того, чтобы получить волшебную палочку, надо выполнить 3 "долгие" асинхронные операции.

3 операции -- если они выполняются успешно -- кнопка "Continue" становится доступной для нажатия.

Это задача для оператора __zip__.

2) Есть 3 переключателя и кнопка "Играть". Чтобы она стала доступна, все переключатели должны быть включены. Но потом один из переключателей может быть оффнут, и кнопка опять станет "серой".

Это поведение описывается оператором __combineLatest__.

Пример использования операторов `zip`, `combineLatest`: https://developer.apple.com/videos/play/wwdc2019/722/

---

Для работы с результатом запроса `URLSession` через `JSONDecoder` есть Combine оператор `decode`.

---

Какие еще есть темы про работу с Combine?

* Error handling & cancellation
* Schedulers & time
* Design Patterns по поводу использования Combine в разных модулях или частях вашего приложения
* Интеграция Combine со SwiftUI.
