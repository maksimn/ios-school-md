...
## Особенности SwiftUI (в сравнении с UIKit).
...
## Элементы в SwiftUI. Layout-контейнеры.
...
## Свойства элемента. Модификаторы в SwiftUI.
...
Так задается __свойство__ `alignment` для элемента.
...
## Стейт-переменные. "Источник правды", что это такое.
...
## ObservableObject, @Published, @StateObject, @ObservedObject, @Binding.

SwiftUI invents a tool called `@Binding` for passing read-write derived values (примера не будет).
